  ...
  return (
    <section aria-labelledby="generator-title" className={styles.section}>
      <div className={styles.headingRow}>
        <div>
          <p className={styles.eyebrow}>Optional one-off direction</p>

          <h2 id="generator-title">Create artwork</h2>
        </div>

        <div className={styles.scopeLock}>
          <span>Original cover ratio</span>
      ...
      <div className={styles.scopeNotice}>
        The Daily Studio prepares the morning collection automatically. Use this
        form only when you want to create one additional artwork outside the
        daily batch. The composition, background, light, size, and final
        typography remain consistent. The review page shows both the artwork and
        its fixed book-cover presentation before you approve it.
      </div>
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        <div className={styles.formGrid}>
          ...
          <label className={styles.field}>
            ...
            <small>Use no more than one subtle symbolic object.</small>
          </label>
          ...
          <label className={styles.field}>
            ...
            <small>
              Use Exploration for first ideas, Refined for stronger versions,
              and Final for approved directions.
            </small>
          </label>
          ...
          <label className={styles.fieldWide}>
            ...
            <small>{moodNotes.length}/279</small>
          </label>
          ...
          <label className={styles.field}>
            ...
            <select
              disabled={isGenerating}
              onChange={(event) =>
                setNumberOfVariations(Number(event.target.value))
              }
        ...
        {!generationEnabled ? (
          <p className={styles.reviewerNotice}>
            Artwork creation is currently unavailable. You can continue
            reviewing and preparing existing artworks.
          </p>
        ) : null}
        ...
        <div className={styles.actions}>
          <button disabled={disabled} type="submit">
            {isGenerating ? "Generating and reviewing…" : "Create artwork"}
          </button>

          <p>
            Every result enters the artwork archive for review. Nothing appears
            in the public gallery until you choose to publish it.
          </p>
        </div>
      </form>
      ...
      {result ? (
        <div className={styles.results}>
          <div className={styles.resultHeader}>
            <div>
              <p className={styles.eyebrow}>Artwork created</p>

              <h3>
                {result.variants.length}{" "}
                {result.variants.length === 1 ? "new artwork" : "new artworks"}
              </h3>
            </div>

            <Link href="/studio/artworks">Open review library</Link>
          </div>
            ...
            {result.variants.map((variant) => (
              <article className={styles.resultCard} key={variant.id}>
                ...
                <div>
                  ...
                  {variant.visualScore !== null ? (
                    <small>
                      Visual score: {variant.visualScore}
                      /100
                    </small>
                  ) : null}
                </div>
